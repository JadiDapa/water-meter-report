  ...
  return (
    <div className="rounded-xl border bg-white p-5 ring-1 ring-sky-200 dark:bg-neutral-900 dark:ring-sky-800">
      <div className="mb-5 flex items-center gap-2.5">
        ...
        <div>
          <p className="text-xs font-medium tracking-widest text-neutral-400 uppercase">Penggunaan Air</p>
          <p className="text-[13px] font-medium text-neutral-500">Pembacaan meteran bulanan</p>
        </div>
      </div>
      <div className="mb-6 flex items-end gap-3">
        ...
        {pct !== null && (
          <div className={cn("mb-1.5 flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold", trendBg, trendColor)}>
            ...
            {isEqual ? "Tidak ada perubahan" : `${pct}% dari bulan lalu`}
          </div>
        )}
      </div>
      <div className="space-y-3">
        <div>
          <div className="mb-1 flex items-center justify-between text-[13px]">
            <span className="font-medium text-neutral-700 dark:text-neutral-300">Bulan ini</span>
        ...
        {previous > 0 && (
          <div>
            <div className="mb-1 flex items-center justify-between text-[13px]">
              <span className="font-medium text-neutral-500">Bulan lalu</span>
      ...
      {previous > 0 && (
        <div className={cn("mt-5 flex items-center gap-2 rounded-lg px-3.5 py-2.5", isEqual ? "bg-neutral-50 dark:bg-neutral-800" : isUp ? "bg-rose-50 dark:bg-rose-950/60" : "bg-emerald-50 dark:bg-emerald-950/60")}>
          ...
          <p className={cn("text-sm font-medium", trendColor)}>
            {isEqual ? "Penggunaan tidak berubah dari bulan lalu." : isUp ? `Penggunaan meningkat ${Math.abs(diff).toLocaleString()} m³ dari bulan lalu.` : `Penggunaan berkurang ${Math.abs(diff).toLocaleString()} m³ dari bulan lalu.`}
          </p>
        </div>
      )}
    </div>
...
function EvidenceGallery({ images }: { images: { url: string }[] }) {
  if (!images.length) return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed bg-neutral-50 py-16 dark:bg-neutral-900">
      ...
      <p className="text-sm text-neutral-400">Tidak ada foto bukti</p>
    </div>
  );
  return (
...
export default async function CustomerReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4 rounded-xl border bg-white p-5 ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-700">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-medium tracking-widest text-neutral-400 uppercase">Bukti Foto</p>
